...
const RestaurantPage = () => {
  ...
  const filteredFoods = FoodsData.filter(
    (food) => food.country === restaurant.country
  )
  ...
  const handleAddToCart = () => {
    const foodToAdd = filteredFoods.find(
      (food) => food.foodId === popupData.foodId
    )
    if (foodToAdd) {
    ...
    <RestaurantPageContainer className="container">
      <RestaurantPageContent>
        {filteredFoods.map((food) => (
          <RestaurantPageCard
            ...
            foodId={food.foodId}
          >
            <RestaurantCardButton onClick={() => handleOpenPopup(food.foodId)}>
              Ver Prato
            </RestaurantCardButton>
          </RestaurantPageCard>
        ))}
      ...
      {showPopup && (
        <>
          ...
          >
            <PopupButton onClick={handleAddToCart}>
              Adicionar ao carrinho - {popupData.price}
            </PopupButton>
          </Popup>
        </>
      )}
